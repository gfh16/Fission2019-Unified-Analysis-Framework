...
void ExtractDEEPointsGUI::DoMarkersFit()
{
  ...
  for (Int_t i=0; i<NUM_POINTS; i++) {
    Energy[i] = CutData[TeleNo][ParticleNo][i][0];  // x
    DeltaE[i] = CutData[TeleNo][ParticleNo][i][1];  // y
  }
  TCanvas* c1_tem = fEcanvas->GetCanvas();
  TGraph* g1 = new TGraph(NUM_POINTS, Energy, DeltaE);
  TF1* fit_func = new TF1("fit_func","[0]/x+[1]+[2]*x+[3]*x*x+[4]*x*x*x+[5]*x*x*x*x+[6]*x*x*x*x*x+[7]*x*x*x*x*x*x",Energy[0]*0.9,Energy[NUM_POINTS-1]*1.1);
  g1->Fit("fit_func","","", Energy[0]*0.9, Energy[NUM_POINTS-1]*1.1);
