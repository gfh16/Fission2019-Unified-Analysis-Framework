...
void test10()
{
  ...
  {
    L1SEChCut[np] = calidata.GetSiEChCut (L1STag.c_str(), SigmaIndex[np]);
    L2FEChCut[np] = calidata.GetSiEChCut (L2FTag.c_str(), SigmaIndex[np]);
    L2BEChCut[np] = calidata.GetSiEChCut (L2BTag.c_str(), SigmaIndex[np]);
    L3AEChCut[np] = calidata.GetCsIEChCut(L3ATag.c_str(), SigmaIndex[np]);

    //if (np!=1) continue;
    for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++)
    {
      for (Int_t ch=0; ch<NUM_STRIP; ch++)
      {
        cout<<"np = "<<np<<setw(10)<<"ssd = "<<ssdindex<<setw(10)<<"ch = "<<ch<<setw(10)
            <<L1SEChCut[np][ssdindex*NUM_STRIP+ch]<<endl;
      }
    }
...
/*
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++)
  {
    for (Int_t np=0; np<NPoints; np++)
    {
    //  cout<<L1SNumOfEffEntries[ssdindex][np]<<endl;
      ...
      cout<<std::setprecision(6)<<L1SMultiRatio[ssdindex][np]<<endl;
    }
  }
*/
