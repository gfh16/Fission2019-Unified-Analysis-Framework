...
void EstimateDeadLayerEffects(const Char_t* layertag)
{
  ...
  Double_t RelativeErr_RangeLow = 0.9*RelativeErrCut;
  Double_t RelativeErr_RangeUp  = 1.1*RelativeErrCut;
  Double_t RelativeErr_RangeLow_L2F = 0.005;
  Double_t RelativeErr_RangeUp_L2F  = 0.015;
  ...
  Double_t deadlayer_Ave_sacle[SSDNum];
  ...
  for (Int_t i=0; i<SSDNum; i++)
  {
    for (Int_t j=0; j<CHNum; j++)
    {
      ...
      for (Int_t k=0; k<Deadlayer_num; k++)
      {
        ...
        } else if ((i<2) && strcmp(layertag,"L2F")==0) { // SSD1_L2F,SSD2_L2F peak2 刻度的斜率
          deadlayer[i][j]   = Sideadlayer_step * 0;      // 比 peak1 的大, 死层修正无效, 因此
          energyloss1[i][j] = E1_ELoss[0];               // 不考虑死层的修正
          energyloss2[i][j] = E2_ELoss[0];
  ...
  for (Int_t i=0; i<SSDNum; i++)
  {
    ...
    {
      SSDCHNum[i][j] = j+1;
      k_RelativeErr[i][j] = abs((c1[i][j]-c2[i][j])/c1[i][j]);
    ...
    deadlayer_Ave_sacle[i]      = (deadlayer_Ave[i]-DeadLayer_RangeLow)*ScaleFactor_DeadLayer + YScaleRangeLow_DeadLayer;
  }
  ...
  for (Int_t i=0; i<SSDNum; i++)
  {
    ...
    hist_scale->SetTitle(Form("SSD%d_%s_Peak1ParameterDistributions",i+1,layertag));
    ...
    if  ((i<2) && strcmp(layertag,"L2F")==0) {
      ...
      k_Err_Yaxis[i]     = new TGaxis(gPad->GetUxmin(),YRangeLow,gPad->GetUxmin(),YScaleRangeUp_Err,
                                      RelativeErr_RangeLow,RelativeErr_RangeUp,1002,"-R");
    }
    deadlayer_Yaxis[i] = new TGaxis(gPad->GetUxmin(),YScaleRangeLow_DeadLayer,gPad->GetUxmin(),YRangeUp,
                                    DeadLayer_RangeLow,DeadLayer_RangeUp,1002,"-R");
    ...
    line_deadlayer_average[i] = new TLine(gPad->GetUxmin(),deadlayer_Ave_sacle[i],
                                    gPad->GetUxmax(),deadlayer_Ave_sacle[i]);
