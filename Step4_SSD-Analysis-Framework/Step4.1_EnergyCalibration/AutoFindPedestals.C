...
// Getting pedestals from file QC_MapSSD_PulserCali_Pedestal0000.root
 /*
  std::string pathRootInput("rootinput/QC_MapSSD_PulserCali_Pedestal0000.root");
  std::string pathPDF("figures/SSD_Pedestals.pdf");
  std::string pathPDFbegin("figures/SSD_Pedestals.pdf[");
  std::string pathPDFend("figures/SSD_Pedestals.pdf]");
  std::string pathAlphaCaliPedestalsOutput("output/SSD_Pedestals.dat");
*/
  ...
  std::string pathPDFOutput(Form("figures/SSD_%s_AlphaCaliPedestals_%s.pdf",L1STag.c_str(), AlphaTag.c_str()));
  std::string pathPDFbegin(Form("figures/SSD_%s_AlphaCaliPedestals_%s.pdf[",L1S.c_str(), AlphaTag.c_str()));
  std::string pathPDFend(Form("figures/SSD_%s_AlphaCaliPedestals_%s.pdf]",L1S.c_str(), AlphaTag.c_str()));
  */


  // Getting pedestals from individual alphacali file
  for (Int_t i=0; i<9; i++) {

    std::string pathRootInput(Form("/data/EXPdata/Fission2019_Data/MapRoot/MapSSD_%s_AlphaCali%04d.root", L1Tag.c_str(), i));

    std::string pathAlphaCaliPedestalsOutput(Form("output/SSD_%s_AlphaCaliPedestals_Alpha%04d.dat", L1STag.c_str(), i));

    std::string pathPDFOutput(Form("figures/SSD_%s_AlphaCaliPedestals_Alpha%04d.pdf", L1STag.c_str(), i));
    std::string pathPDFbegin(Form("figures/SSD_%s_AlphaCaliPedestals_Alpha%04d.pdf[", L1STag.c_str(), i));
    std::string pathPDFend(Form("figures/SSD_%s_AlphaCaliPedestals_Alpha%04d.pdf]", L1STag.c_str(), i));
  ...
  for (Int_t SSDNum=0; SSDNum<4; SSDNum++)
  {
    ...
    {
      Pedestals[SSDNum][CHNum] = (TH1D*)FileIn->Get(Form("SSD%d_%s_E_CH%02d",SSDNum+1,L1STag.c_str(),CHNum));
