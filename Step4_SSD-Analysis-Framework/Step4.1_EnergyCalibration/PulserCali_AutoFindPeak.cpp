...
Double_t fpeaks(Double_t *x, Double_t *par)
{
  ...
  {
    if(par[3*p]<0||par[3*p+1]<0||par[3*p+2]<0) continue;
    Double_t norm   = par[3*p];      // "height"
    Double_t mean   = par[3*p+1];    // "mean"
    Double_t sigma  = par[3*p+2];    // "sigma"
...
//////////////////////////////////////////////////////////////
  ...
  for(int i=0; i<16; i++)
  {
    ...
    // if one Histogram is empty, then continue
    if(PulserPeaks[i]==0)
    {
    ...
    TF1 *fit = new TF1("fit",fpeaks,0,4095,3*npeaks);
