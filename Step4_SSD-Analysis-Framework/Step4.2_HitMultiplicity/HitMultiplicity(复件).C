...
void HitMultiplicity()
{
  ...
  std::string pathPedestalCut(Form("../Step4.1_EnergyCalibration/output/SSD_%s_PulserPedestals.dat",LayerTag.c_str()));
  ...
  TCanvas* cans = new TCanvas("cans","cans",CanvasWidth,CanvasHeight);
  cans->Divide(2,2);
  ...
  for (Int_t i=0; i<SSDNum; i++)
  {
    SSD_E_bname[i] = Form("SSD%d_%s_E", i+1, LayerTag.c_str());
    myChain->SetBranchStatus(SSD_E_bname[i].c_str(), true);
    myChain->SetBranchAddress(SSD_E_bname[i].c_str(), SSD_E[i]);

    SSD_E_HitMulti_histname[i] = Form("SSD%d_%s_HitMulti_Run%04d-%04d",i+1,LayerTag.c_str(),FirstRun,LastRun);
