...
// Declaration of leaf types
  ...
  CSHINEHitPatternRecognition(TTree* tree=0);
  virtual ~CSHINEHitPatternRecognition();

  virtual void Init(TTree* tree);

  // 在这里定义需要进行的操作
  Int_t  LayerMultiplicity(const char* layertag, Int_t* ech, Int_t numch, Int_t numsigma);
};
...
//=============================
#ifdef CSHINEHitPatternRecognition_cxx
//______________________________________________________________________________
CSHINEHitPatternRecognition::CSHINEHitPatternRecognition(TTree* tree) : fChain(0)
{
  Init(tree);
}
...
//______________________________________________________________________________
void CSHINEHitPatternRecognition::Init(TTree* tree)
{
  if (!tree) return;
  fChain = tree;
