...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (SSD1_fL1SMulti > 8 || SSD1_fL1SMulti==0) continue;

      for (Int_t csich=0; csich<SSD1_fCsIMulti; csich++) {
        Int_t fGlobalMulti = 0;
        for (Int_t stripl2b=0; stripl2b<SSD1_fL2BMulti; stripl2b++) {
          if (pattern.GeoConstraint_L3A_L2B(SSD1_fCsINum[csich], SSD1_fL2BNumStrip[stripl2b])) {
            for (Int_t stripl2f=0; stripl2f<SSD1_fL2FMulti; stripl2f++) {
              if (pattern.EnergyConstraint_L2B_L2F(SSD1_fL2BEMeV[stripl2b], SSD1_fL2FEMeV[stripl2b], 5.)
              && pattern.GeoConstraint_L3A_L2F(SSD1_fCsINum[csich], SSD1_fL2FNumStrip[stripl2f])) {
                for (Int_t stripl1s=0; stripl1s<SSD1_fL1SMulti; stripl1s++) {
                  if (pattern.GeoConstraint_L2B_L1S(SSD1_fL2BNumStrip[stripl2b], SSD1_fL1SNumStrip[stripl1s])) {
                    fGlobalMulti++;
                    ...
                    cout<<setw(10)<<"CSI = "<<setw(2)<<SSD1_fCsINum[csich]
                    ...
                    cout<<setw(10)<<"EL1S = "<<setw(5)<<SSD1_fL1SEMeV[stripl1s]<<endl;
                  }
                }
              }
            }
        ...
        /*
        cout<<right;
        cout<<setw(10)<<"M_L3A = "<<SSD1_fCsIMulti<<setw(12)<<"M_L2B = "<<SSD1_fL2BMulti
            <<setw(12)<<"M_L2F = "<<SSD1_fL2FMulti<<setw(12)<<"M_L1S = "<<SSD1_fL1SMulti
            <<setw(16)<<"MultiGlobal = "<<fGlobalMulti
            <<endl; */
      }
   }
}
