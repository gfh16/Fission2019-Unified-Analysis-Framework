  ...
  Int_t numtel = ssdindex*NUM_CSI + csiindex; // CsI 编号: 0-35
    ...
    for (Int_t ip=0; ip<numparticle; ip++) {
          ...
          dist       = dE - DoCalcdEMeV(EStoped,L2L3_DEEFitPars[numtel][ip-1],NFITPARS);
					...
					if (pidnum<pidnum_low || pidnum>pidnum_up) {
						pidnum = pidnum_up; // 如果算出来的 pid 不在两条标准先之内,就使用所在 cut 对应的 pid
					}
				}
					...
					pidnum     = pidnum_low + dist/dErange*(pidnum_up-pidnum_low);
					if (pidnum<pidnum_low || pidnum>pidnum_up) {
						pidnum = pidnum_low; // 如果算出来的 pid 不在两条标准先之内,就使用所在 cut 对应的 pid
					//	printf("ssd=%d pid=%.4f, pid_low=%.4f, pid_up=%.4f, plow=(%d,%d), pup=(%d,%d)\n", ssdindex,
					//	       pidnum, pidnum_low, pidnum_up,L2L3_Charge[numtel][ip],L2L3_Mass[numtel][ip],L2L3_Charge[numtel][ip+1],L2L3_Mass[numtel][ip+1]);
					}
				}
			}
		}
...
//______________________________________________________________________________
Int_t CSHINEParticleIdentificationSimplified::GetZoneOfPixel(Int_t stripb, Int_t stripf, Int_t ZoneLength) const
{
	Int_t num_merged_chs = NUM_STRIP/ZoneLength;
	Int_t zone_x = std::abs(stripb/num_merged_chs - (ZoneLength-1));
	Int_t zone_y = std::abs(stripf/num_merged_chs - (ZoneLength-1));
