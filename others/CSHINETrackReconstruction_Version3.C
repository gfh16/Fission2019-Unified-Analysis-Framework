...
void DecodefGlobalMulti1::GlobalMulti1_L2L3_Decode()
{
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    h1_candimulti_sum[ssdindex] = new TH1I(Form("SSD%d_CandiMulti",ssdindex+1), Form("SSD%d_CandiMulti",ssdindex+1), 3, -0.5, 2.5);
    h1_candimulti_merged[ssdindex] = new TH1I(Form("SSD%d_CandiMulti_Merged",ssdindex+1), Form("SSD%d_CandiMulti_Merged",ssdindex+1), 3, -0.5, 2.5);
    for (Int_t csiindex=0; csiindex<NUM_CSI; csiindex++) {
      ...
      h1_candimulti[ssdindex][csiindex] = new TH1I(Form("SSD%d_CsI%d_CandiMulti",ssdindex+1,csiindex), Form("SSD%d_CsI%d_CandiMulti",ssdindex+1,csiindex), 3, -0.5, 2.5);
    }
  }
  ...
  for (Long64_t ientry=0; ientry<nentries; ientry++) {
    ...
    for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        //           以下部分为当前 L2L3-globalomulti21 的径迹重建策略！！！
        //          -------------------------------------------------------
