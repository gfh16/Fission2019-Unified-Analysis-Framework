...
//______________________________________________________________________________
// Decode 策略:
//  case 1) bin_index = 111 = 7 : 直接判选
//  case 2) bin_index = 101 = 5 : 反推 EL1S
//  case 3) bin_index = others  : 舍弃
void DecodefGlobalMulti2::GlobalMulti2_Decode_1111()
{
  const Int_t globalmulti = 2;
  const char* mode = "1111";
  std::string mode_exp;
  Double_t Condition_101_L2FElossCut = 4.0; // MeV
  Double_t Condition_101_L2FEincCut  = 0.01; // MeV
  ...
  Int_t Entry_Corrected[NUM_SSD] = {0};
  Int_t index_corrected[NUM_SSD];
  ...
  TH1I* h1_submode101[NUM_SSD][NUM_CSI];
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    ...
    for (Int_t csiindex=0; csiindex<NUM_CSI; csiindex++) {
      ...
      h1_submode101[ssdindex][csiindex] = new TH1I(Form("SSD%d_CsI%d_Submode101_%s",ssdindex+1,csiindex,mode), Form("SSD%d_CsI%d_Submode101_%s",ssdindex+1,csiindex,mode), 8, -0.5, 7.5);
  ...
  // trackreconstruct->fChainTrackTree->SetEntries(100);
  Long64_t nentries = trackreconstruct->fChainTrackTree->GetEntries();
  cout<<"Found nentries = "<<nentries<<endl;
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        if (strcmp(mode_exp.c_str(),mode) == 0) {
          for (Int_t itrack=0; itrack<globalmulti; itrack++) {
            ...
            // 根据三个判选条件, 定义 sub-mode
            bin_index[itrack] = GlobalMulti2_ModeStatistic(trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]),
                                                           trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]),
                                                           trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][itrack]], cutname, L2L3_CSIECh[ssdindex][itrack],L2L3_L2FEMeV[ssdindex][itrack]));
            ...
            //          -------------------------------------------------------
            // 对于 bin_index == 111 = 7 的情况: 直接填充
            if (bin_index[itrack] == 7) { // sub_mode = 111
            ...
            // 对于 bin_index = 101 = 5 的情况: 考虑 EL1S 修正
            else if (bin_index[itrack] == 5) {
              trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
              if (L2L3_L2FEMeV[ssdindex][itrack]>Condition_101_L2FElossCut) {
                L2F_Einc = fEloss.GetIncidentEnergy(charge,mass,L2L3_L2FEMeV[ssdindex][itrack],"Si",SIL2THICKNESS[ssdindex]*1000,1);
                if (L2F_Einc > Condition_101_L2FEincCut) {
                  L1S_ELoss = fEloss.GetEnergyLossFromResidualEnergy(charge,mass,L2F_Einc,"Si",SIL1THICKNESS[ssdindex]*1000,1);
                  if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L1S_ELoss, L2L3_L2FEMeV[ssdindex][itrack])) {
                    h2_L2L3_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_CSIECh[ssdindex][itrack],L2L3_L2FEMeV[ssdindex][itrack]);
                    h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_L2FEMeV[ssdindex][itrack],L1S_ELoss);
                    h1_submode101[ssdindex][L2L3_CsINum[ssdindex][itrack]]->Fill(bin_index[itrack]);
                    index_corrected[ssdindex]++;
                  }
                  else {
            ...
            // 对于 bin_index != 111 = 7 且 bin_index != 101 = 5的情况: 舍弃
            else {
              h2_L2L3_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_CSIECh[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]);
              h2_L1L2_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_L2FEMeV[ssdindex][itrack],L2L3_L1SEMeV[ssdindex][itrack]);
            }
          ...
          // 计算找回的 entry 数目
          if (candimulti_sum[ssdindex]==0 && index_corrected[ssdindex]>0)  Entry_Corrected[ssdindex]++;
  ...
  // 计算比例
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    Double_t entry_all = h1_candimulti_sum[ssdindex]->GetEntries();
    Double_t entry_discard = h1_candimulti_sum[ssdindex]->GetBinContent(1) - Entry_Corrected[ssdindex];
    lost_ratio[ssdindex] = entry_discard/entry_all *100;
  }
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    for (Int_t csiindex=0; csiindex<NUM_CSI; csiindex++) {
      ...
      h1_submode101[ssdindex][csiindex]->Draw("SAME");
      h1_submode101[ssdindex][csiindex]->SetLineColor(kRed);
      h1_submode101[ssdindex][csiindex]->SetLineWidth(2);
...
void DecodefGlobalMulti2::GlobalMulti2_Decode_1101()
{
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        if (strcmp(mode_exp.c_str(),mode) == 0) {
          for (Int_t itrack=0; itrack<globalmulti; itrack++) {
            ...
            bin_index[itrack] = GlobalMulti2_ModeStatistic(trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]),
                                                           trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]),
                                                           trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][itrack]], L2L3_CSIECh[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]));
            h1_modestat[ssdindex][L2L3_CsINum[ssdindex][itrack]]->Fill(bin_index[itrack]);

            // 计算每个 entry 下的 candidate multi
          ...
          if (candimulti_sum[ssdindex] == 0) {
            ...
            if (trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, EL2B_Sum, L2L3_L2FEMeV[ssdindex][0])) {
              for (Int_t itrack=0; itrack<globalmulti; itrack++) { // 这种情况下,使用 EL2B 代替 EL2F
                if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][itrack], L2L3_L2BEMeV[ssdindex][itrack]) &&
                    trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][itrack]], L2L3_CSIECh[ssdindex][itrack],L2L3_L2BEMeV[ssdindex][itrack]))
                {
          ...
          // 计算找回的 entry 数目
          if (index_corrected[ssdindex] > 0)  Entry_Corrected[ssdindex]++;
        }
      }
    }
  }
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    Double_t entry_all = h1_candimulti_sum[ssdindex]->GetEntries();
    Double_t entry_discard = h1_candimulti_sum[ssdindex]->GetBinContent(1) + h1_candimulti_sum[ssdindex]->GetBinContent(3) - Entry_Corrected[ssdindex];
    lost_ratio[ssdindex] = entry_discard/entry_all *100;
  }
...
Bool_t IsTwoAdjCsI_GetTheMax(vector<Int_t> ecsi, vector<Int_t> csinum, Int_t& getitrack, Double_t ecut)
{
  Bool_t index = kFALSE;

  // 两块 CsI 相邻
  if (((csinum[0]/3 == csinum[1]/3)&&(csinum[0]-csinum[1] == -1)) || ((csinum[0]%3 == csinum[1]%3)&&(csinum[0]-csinum[1] == -3))) {
    if (ecsi[0] >= ecsi[1]) {
      if (ecsi[0]/(ecsi[0]+ecsi[1]) >= ecut) {
        index = kTRUE;
        getitrack = 0;
      }
      else {
...
void DecodefGlobalMulti2::GlobalMulti2_Decode_1010()
{
  ...
  std::string mode_exp;
  Double_t Condition_L2FElossCut = 4.0; // MeV
  Double_t Condition_L2FEincCut  = 0.1; // MeV
    ...
    for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        if (strcmp(mode_exp.c_str(),mode) == 0) {
          ...
          // 存在 0 个“理想事件“的的情况, 检查 L2F sharing 的比例
          if (candimulti_sum[ssdindex] == 0) {
          ...
          if (candimulti_sum[ssdindex] == 0) {
            ...
            if (trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex,L2L3_L2BEMeV[ssdindex][0],EL2F_Sum) &&
                trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][0]],L2L3_CSIECh[ssdindex][0],L2L3_L2FEMeV[ssdindex][0]) &&
                trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][1]],L2L3_CSIECh[ssdindex][1],L2L3_L2FEMeV[ssdindex][1]))
            {
              trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
              for (Int_t itrack=0; itrack<globalmulti; itrack++) {
            ...
            // 一条真实径迹, L2F 中有 sharing
            if (TMath::Abs(L2L3_L2FStripNum[ssdindex][0]-L2L3_L2FStripNum[ssdindex][1])==1 &&
                trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][0], EL2F_Sum)
                trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][0], EL2F_Sum)) {
              if (!(trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][0]],L2L3_CSIECh[ssdindex][0],EL2F_Sum) &&
                    trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][1]],L2L3_CSIECh[ssdindex][1],EL2F_Sum)))
              {
                for (Int_t itrack=0; itrack<globalmulti; itrack++) {
                  if (trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][itrack]],L2L3_CSIECh[ssdindex][itrack],EL2F_Sum)) {
                    h2_L2L3_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_CSIECh[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]);
                    h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_L2FEMeV[ssdindex][itrack], L2L3_L1SEMeV[ssdindex][itrack]);
                    h1_SubmodeCorrected[ssdindex][L2L3_CsINum[ssdindex][itrack]]->Fill(bin_index[itrack]);
                    index_corrected[ssdindex]++;
  ...
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    Double_t entry_all = h1_candimulti_sum[ssdindex]->GetEntries();
    Double_t entry_discard = h1_candimulti_sum[ssdindex]->GetBinContent(1) + h1_candimulti_sum[ssdindex]->GetBinContent(3);
    lost_ratio[ssdindex] = entry_discard/entry_all *100;
  }
...
// Decode 策略:
//  case 1) candimulti = 1   : 直接判选
//  case 2) candimulti = 0/2 : 考虑 L2B sharing 等
void DecodefGlobalMulti2::GlobalMulti2_Decode_0100()
{
  ...
  Double_t Condition_L2FEincCut  = 0.01; // MeV
    ...
    for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        if (strcmp(mode_exp.c_str(),mode) == 0) {
          ...
          // 存在 0/2 个“理想事件“的的情况 : 进一步考虑 L2B sharing 等
          if (candimulti_sum[ssdindex] == 0 || candimulti_sum[ssdindex] == 2) {
            ...
            if (trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][0]], cutname, L2L3_CSIECh[ssdindex][0], L2L3_L2FEMeV[ssdindex][0]))
            {
                  ...
                  !trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][1], L2L3_L2FEMeV[ssdindex][1]))
              {
                ...
                // 如果 EL1S 不满足条件, 则反推 EL1S, 再判断是否满足条件
                else {
                  trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
                  if (L2L3_L2FEMeV[ssdindex][0]>Condition_L2FElossCut) {
                    ...
                    if (L2F_Einc > Condition_L2FEincCut) {
                      L1S_ELoss = fEloss.GetEnergyLossFromResidualEnergy(charge,mass,L2F_Einc,"Si",SIL1THICKNESS[ssdindex]*1000,1);
                      if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L1S_ELoss, L2L3_L2FEMeV[ssdindex][0])) {
                        ...
                        trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][1], L2L3_L2FEMeV[ssdindex][1]))
              {
                ...
                // 如果 EL1S 不满足条件, 则反推 EL1S, 再判断是否满足条件
                  ...
                  if (L2L3_L2FEMeV[ssdindex][1]>Condition_L2FElossCut) {
                    ...
                    if (L2F_Einc > Condition_L2FEincCut) {
                      L1S_ELoss = fEloss.GetEnergyLossFromResidualEnergy(charge,mass,L2F_Einc,"Si",SIL1THICKNESS[ssdindex]*1000,1);
                      if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L1S_ELoss, L2L3_L2FEMeV[ssdindex][1])) {
                ...
                //
                h1_SubmodeCorrected[ssdindex][L2L3_CsINum[ssdindex][1]]->Fill(bin_index[1]);
                index_corrected[ssdindex]++;
              }
              // 两条候选径迹都不满足判选条件, 考虑 L2B sharing
              else if (!trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][0], L2L3_L2FEMeV[ssdindex][0]) &&
                       !trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][1], L2L3_L2FEMeV[ssdindex][1]))
              {
                ...
                else { // 两条候选径迹都不满足判选条件, 同时不满足 L2B sharing (这一步会导致丢掉少部分事件)
                  h2_L2L3_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_CSIECh[ssdindex][0], L2L3_L2FEMeV[ssdindex][0]);
                  h2_L1L2_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_L2FEMeV[ssdindex][0], L2L3_L1SEMeV[ssdindex][0]);
                }
              }
...
// Decode 策略:
//  case 1) candimulti = 1   : 直接判选
//  case 2) candimulti = 0/2 : 考虑 L2F sharing
    ...
    for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
      ...
      if (L2L3_CsINum[ssdindex].size() == globalmulti) {
        ...
        if (strcmp(mode_exp.c_str(),mode) == 0) {
          ...
          // 对于 0/2 个理想事件的情况, 考虑 charge sharing
          if (candimulti_sum[ssdindex] == 0 || candimulti_sum[ssdindex] == 2) {
                ...
                !trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][1]], cutname, L2L3_CSIECh[ssdindex][1], L2L3_L2FEMeV[ssdindex][1]))
            {
              ...
              if (trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][0], L2L3_L2FEMeV[ssdindex][0])) {
                h2_L2L3_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_CSIECh[ssdindex][0],L2L3_L2FEMeV[ssdindex][0]);
                // 若满足 EL1S_EL2F Cut, 直接填充
                if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][0], L2L3_L2FEMeV[ssdindex][0])) {
                  h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_L2FEMeV[ssdindex][0],L2L3_L1SEMeV[ssdindex][0]);
                }
                // 若不满足 EL1S_EL2F Cut, 反推 EL1S
                else {
                  trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
                  if (L2L3_L2FEMeV[ssdindex][0]>Condition_L2FElossCut) {
                    L2F_Einc = fEloss.GetIncidentEnergy(charge,mass,L2L3_L2FEMeV[ssdindex][0],"Si",SIL2THICKNESS[ssdindex]*1000,1);
                      ...
                      trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][1]], cutname, L2L3_CSIECh[ssdindex][1], L2L3_L2FEMeV[ssdindex][1]))
            {
              ...
              if (trackreconstruct->IsEneConstraint_L2B_L2F(ssdindex, L2L3_L2BEMeV[ssdindex][1], L2L3_L2FEMeV[ssdindex][1])) {
                h2_L2L3_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][1]].Fill(L2L3_CSIECh[ssdindex][1],L2L3_L2FEMeV[ssdindex][1]);
                // 若满足 L1S_L2F Cut, 直接填充
                if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L2L3_L1SEMeV[ssdindex][1], L2L3_L2FEMeV[ssdindex][1])) {
                  h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][1]].Fill(L2L3_L2FEMeV[ssdindex][1],L2L3_L1SEMeV[ssdindex][1]);
                }
                // 不满足 L1S_L2F Cut, 反推 EL1S
                else {
                  trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
                  if (L2L3_L2FEMeV[ssdindex][1]>Condition_L2FElossCut) {
                    L2F_Einc = fEloss.GetIncidentEnergy(charge,mass,L2L3_L2FEMeV[ssdindex][1],"Si",SIL2THICKNESS[ssdindex]*1000,1);
            ...
            // 两条候选径迹都不满足 Banana Cut
            else if (!trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][0]], cutname, L2L3_CSIECh[ssdindex][0], L2L3_L2BEMeV[ssdindex][0]) &&
            ...
            {
              // 考虑 L2F charge sharing 后, 满足 EL2B_EL2F Cut
              if (TMath::Abs(L2L3_L2FStripNum[ssdindex][0]-L2L3_L2FStripNum[ssdindex][1])==1 &&
                ...
                // 不满足 EL1S_EL2F Cut, 反推 EL1S
                else {
                  trackreconstruct->GetZAFromBananaCutName(cutname,charge,mass);
                  if (EL2F_sharing>Condition_L2FElossCut) {
                    L2F_Einc = fEloss.GetIncidentEnergy(charge,mass,EL2F_sharing,"Si",SIL2THICKNESS[ssdindex]*1000,1);
                    if (L2F_Einc > Condition_L2FEincCut) {
                      L1S_ELoss = fEloss.GetEnergyLossFromResidualEnergy(charge,mass,L2F_Einc,"Si",SIL1THICKNESS[ssdindex]*1000,1);
                      if (trackreconstruct->IsEneConstraint_L1S_L2F(ssdindex, L1S_ELoss, EL2F_sharing)) {
                        h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(EL2F_sharing,L2L3_L1SEMeV[ssdindex][0]);
                      }
                    }
                  }
              ...
              // 考虑 L2F charge sharing 后, 不满足 EL2B_EL2F Cut
              else {
                for (Int_t itrack=0; itrack<globalmulti; itrack++) {
                  h2_L2L3_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_CSIECh[ssdindex][itrack], L2L3_L2FEMeV[ssdindex][itrack]);
                  h2_L1L2_DEE_Discard[ssdindex][L2L3_CsINum[ssdindex][itrack]].Fill(L2L3_L2FEMeV[ssdindex][itrack], L2L3_L1SEMeV[ssdindex][itrack]);
...
void DecodefGlobalMulti2::GlobalMulti2_Decode_0001()
{
  ...
  TH1I* h1_L1SELISE[NUM_SSD][NUM_CSI];
  TH1I* h1_candimulti[NUM_SSD][NUM_CSI];
  TH1I* h1_candimulti_sum[NUM_SSD];
  TH1I* h1_candimulti_merged[NUM_SSD];
  for (Int_t ssdindex=0; ssdindex<NUM_SSD; ssdindex++) {
    ...
    for (Int_t csiindex=0; csiindex<NUM_CSI; csiindex++) {
      ...
      h1_L1SELISE[ssdindex][csiindex] = new TH1I(Form("SSD%d_CsI%d_L1SELISE_%s",ssdindex+1,csiindex,mode), Form("SSD%d_CsI%d_L1SELISE_%s",ssdindex+1,csiindex,mode), 8, -0.5, 7.5);
      h1_candimulti[ssdindex][csiindex] = new TH1I(Form("SSD%d_CsI%d_CandiMulti_%s",ssdindex+1,csiindex,mode), Form("SSD%d_CsI%d_CandiMulti_%s",ssdindex+1,csiindex,mode), 3, -0.5, 2.5);
    }
  }
            ...
            if (trackreconstruct->IsInsideABananaCut(bananacut[BananaCut_TelNum[ssdindex][0]],cutname,L2L3_CSIECh[ssdindex][0],L2L3_L2FEMeV[ssdindex][0])) {
              ...
              if (L2L3_L2FEMeV[ssdindex][0]>Condition_L2FElossCut) {
                ...
                if (L2F_Einc > Condition_L2FEincCut) {
                  ...
                  // 如果两个 EL1S 与 计算值 L1S_ELoss 相差较小, 则保留第一个
                  else if (trackreconstruct->IsEneConstraint_EL1S_ELISECalc(L2L3_L1SEMeV[ssdindex][0],L1S_ELoss,L1S_ECalc_EReal_ErrCut) &&
                           trackreconstruct->IsEneConstraint_EL1S_ELISECalc(L2L3_L1SEMeV[ssdindex][1],L1S_ELoss,L1S_ECalc_EReal_ErrCut)) {
                    h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_L2FEMeV[ssdindex][0],L1S_ELoss);
                    //
                  ...
                  // 如果两个 EL1S 与 计算值 L1S_ELoss 相差较大, 则考虑 L1S charge sharing
                    ...
                    // 否则使用计算值 L1S_ELoss
                      ...
                      h2_L1L2_DEE_EneL2BL2F_EneL1SL2F_PID[ssdindex][L2L3_CsINum[ssdindex][0]].Fill(L2L3_L2FEMeV[ssdindex][0],L1S_ELoss);
                      for (Int_t itrack=0; itrack<globalmulti; itrack++) {
                        h1_L1SELISE[ssdindex][L2L3_CsINum[ssdindex][itrack]]->Fill(bin_index[itrack]);
                      }
                    }
                  }
                }
